...
/**
 * Defines the web service endpoint for signing up to a Tutorial booking activity.
 *
...
class signup extends \external_api {
    /**
     * Sign up for a tutorial booking slot.
     *
     ...
     * @return string
     */
    public static function signup($slotid) {
        global $DB, $USER;
        $params = self::validate_parameters(self::signup_parameters(), array('slotid' => $slotid));
        $return = array();
        ...
        try {
            \mod_tutorialbooking_user::adduser(
                    $USER->id,
                    $session,
                    $tutorial,
                    $context,
     ...
     */
    public static function signup_parameters() {
     ...
     * @return \external_function_parameters
     */
    public static function signup_returns() {
        return new \external_function_parameters(array(
            'success' => new \external_value(PARAM_BOOL, 'Returns is the sign up was a success', VALUE_REQUIRED),
